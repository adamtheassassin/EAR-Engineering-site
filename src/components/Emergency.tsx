...
export default function Emergency({ onOpenModal }: EmergencyProps) {
    return (
        <section className="group relative py-6 md:py-8 overflow-hidden bg-black border-y border-gray-800 shadow-[0_10px_30px_rgba(0,0,0,0.5)] z-20">
            {/* Background Effects */}
            <div className="absolute inset-0 pointer-events-none overflow-hidden">

                {/* Shimmer light beam effect on hover */}
                <div className="absolute inset-0 -translate-x-[150%] bg-gradient-to-r from-transparent via-white/5 to-transparent group-hover:translate-x-[150%] transition-transform duration-[1500ms] ease-in-out pointer-events-none"></div>
            </div>
            ...
            <div className="container mx-auto px-4 relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8">
                ...
                <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-4 md:gap-6 flex-1">
                    {/* Glowing Icon */}
                    <div className="flex-shrink-0 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#FFCA08]/10 shadow-inner border border-[#FFCA08]/30 backdrop-blur-xl relative">
                        <div className="absolute inset-0 bg-[#FFCA08]/20 rounded-2xl animate-ping opacity-25"></div>
                        <svg className="h-7 w-7 text-[#FFCA08] drop-shadow-md" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                    </div>

                    <div className="flex flex-col justify-center">
                        <h2 className="text-xl md:text-2xl lg:text-3xl font-black tracking-tight text-[#FFCA08] drop-shadow-sm leading-tight">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3 mt-1 md:mt-1.5">
                            ...
                            <div className="inline-flex items-center justify-center md:justify-start gap-1.5 opacity-85 text-xs md:text-sm font-medium text-gray-400">
                                <svg className="h-4 w-4 text-[#FFCA08]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
